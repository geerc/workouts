...
aug.11.2017 <- read.csv("~/GitHub/workouts/Cruise Miles/Aug 11 2017.csv")
aug.24.2017 <- read.csv("~/GitHub/workouts/Cruise Miles/Aug 24 2017.csv")
aug.29.2017 <- read.csv("~/GitHub/workouts/Cruise Miles/Aug 29 2017.csv")
oct.25.2016 <- read.csv("~/GitHub/workouts/Cruise Miles/Oct 25 2016.csv")

# Copy Time data to Moving.Time, so we can delete rest splits
aug.11.2017 <- mutate(aug.11.2017, Moving.Time = Time)
aug.24.2017 <- mutate(aug.24.2017, Moving.Time = Time)
...
aug.24.2017 <- aug.24.2017[-c(2, 4, 6, 8, 9), ] %>% select(Split, Moving.Time)
...
names(aug.24.2017)[names(aug.24.2017) == "Moving.Time"] <- "Time"
...
# rownames(aug.24.2017) <- 1:nrow(aug.24.2017)
rownames(aug.29.2017) <- 1:nrow(aug.29.2017)
rownames(oct.25.2016) <- 1:nrow(oct.25.2016)


# Join wokrouts together
first.join <- full_join(aug.11.2017, aug.29.2017, by = "Split")
# second.join <- full_join(aug.29.2017, oct.25.2016, by = "Split")
all.workouts <- full_join(first.join, oct.25.2016, by = "Split")
rm(first.join, second.join)
...
names(all.workouts)[names(all.workouts) == "Time.y"] <- "Aug 29 2017"
names(all.workouts)[names(all.workouts) == "Time"] <- "Oct 25 2016"

# Make new data table optimized for plotting
all.workouts.plot <- all.workouts %>% gather(`Oct 25 2017`, `Aug 11 2017`, `Aug 29 2017`, key = "Date", value = "Time") %>% arrange(Split)
